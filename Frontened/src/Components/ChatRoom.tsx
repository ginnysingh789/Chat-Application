...
export const ChatRoom = () => {
    ...
    const sendMessageToServer = () => {
        if (socketRef.current && text.trim()) {
            ...
            setMessages((prev) => [
                ...prev,
                { text, isSent: true, timestamp: new Date() },
            ])
        ...
        <div className="flex justify-center h-screen w-full fixed p-4 bg-gradient-to-br from-emerald-500 to-teal-700">
            <Card className="w-full max-w-md flex flex-col overflow-hidden shadow-2xl rounded-xl">
                <div className="bg-emerald-600 text-white p-3 flex justify-between items-center shadow-md">
                    ...
                    <div>
                        Room: <span className="font-bold bg-white bg-opacity-20 px-2 py-1 rounded-md">{code}</span>
                    </div>
                </div>
                        ...
                        <div key={groupIndex} className="mb-4">
                            ...
                            {group.messages.map((msg, index) => (
                                <div key={index} className={`flex ${msg.isSent ? "justify-end" : "justify-start"} mb-2 animate-fadeIn`}>
                                    <div
                                        className={`max-w-[80%] rounded-lg px-3 py-2 shadow-sm ${msg.isSent ? "bg-emerald-100 rounded-tr-none" : "bg-white rounded-tl-none"
                                            }`}
                                    >
                ...
                <div className="bg-gray-100 p-2 flex items-end gap-2 border-t border-gray-200">
                    <button className="text-gray-500 hover:text-emerald-600 transition-colors"><Smile size={20} /></button>
                    <button className="text-gray-500 hover:text-emerald-600 transition-colors"><Paperclip size={20} /></button>
                    <textarea
                        ref={textareaRef}
                        value={text}
                    ...
                    <button
                        ...
                        className={`${text.trim() ? "bg-emerald-600" : "bg-gray-400"} text-white p-2 rounded-full h-10 w-10 flex items-center justify-center transition-colors duration-200 hover:bg-emerald-700`}
                        disabled={!text.trim()}
                    >
